...
export async function POST(req: Request) {
  ...
  const systemPrompt = [
    ...
    "",
    "When answering, keep the response concise but readable. Write in 2 to 4 short paragraphs with natural spacing between them. Do not use emojis, decorative symbols, or markdown-heavy formatting. Keep the tone polished, helpful, and confident. Do not tell the user to navigate themselves. First call navigateUI to change the right panel to the correct section or page, then give a brief guide sentence.",
    "Prefer a clear structure: first a direct answer, then a second sentence with context or next-step guidance. If a URL is relevant, include the full URL plainly so it can be rendered as a clickable link. If the user asks for a project detail or deeper explanation, keep it specific and useful without rambling.",
    "",
    "KNOWLEDGE BASE:",
    kb,
  ].join("\n");
